	...
	<div class="w-full overflow-hidden h-[400px]">
		...
		<div v-else>
			<div class="relative overflow-hidden">
				...
				<div :key="activeIndex"
					class="w-full grid grid-cols-1 lg:grid-cols-4 gap-1 sm:gap-0 px-2 justify-center text-center overflow-y-auto max-h-[400px] sm:justify-items-center sm:px-0 sm:py-2">
					<ProductCardSmall v-for="prod in currentSlide"
						:key="prod.codigo"
						v-lazy-src
						:product="prod"/>
				</div>
				...
				<div v-if="slides.length > 1"
                    ...
                    <span v-for="(s, i) in slides"
						:key="i"
						:class="i === activeIndex ? 'bg-amber-300 opacity-80' : 'bg-white opacity-40'"
